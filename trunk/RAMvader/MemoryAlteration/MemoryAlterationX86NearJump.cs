...
namespace RAMvader.CodeInjection
{
	...
	///    memory (which is the default behaviour of the <see cref="Injector{TMemoryAlterationSetID, TCodeCave, TVariable}"/> class). If you need to jump to a code cave in automatically-allocated memory spaces,
	///    you should strongly consider using <see cref="MemoryAlterationX86FarJump"/> instead of <see cref="MemoryAlterationX86NearJump"/>.
	/// </summary>
	public class MemoryAlterationX86NearJump : MemoryAlterationBase
	{
		...
		///    whenever <see cref="MemoryAlterationX86NearJump.SetEnabled{TMemoryAlterationID, TCodeCave, TVariable}(Injector{TMemoryAlterationID, TCodeCave, TVariable}, bool)"/> is called to deactivate a memory alteration.
		/// </param>
		...
		public MemoryAlterationX86NearJump( RAMvaderTarget targetIORef, MemoryAddress targetAddress,
			...
			Injector<TMemoryAlterationID, TCodeCave, TVariable> injectorRef, bool bEnable )
		{
			...
			if ( bEnable )
				return injectorRef.WriteX86NearJumpInstruction( m_jumpType, this.TargetAddress,
					new InjectedCodeCaveMemoryAddress<TMemoryAlterationID, TCodeCave, TVariable>( injectorRef, (TCodeCave) (Object) m_codeCaveID ), this.TargetOriginalBytes.Length );
